...
if (process.argv.includes("--docs")) {
  for (const name of ["livekit_docs", "openaiDeveloperDocs"]) {
    ...
    try {
      await client.connect(
        new StreamableHTTPClientTransport(new URL(config.mcp_servers[name].url)),
        { timeout: 30_000 },
      );
  ...
  const server = createServer((_request, response) => {
    ...
    response.end(
      '<!doctype html><html lang="en"><meta charset="utf-8"><title>AI tooling check</title><main><h1>AI tooling check</h1><p>This is a browser tooling fixture, separate from the application.</p><button type="button" onclick="this.textContent=\'Interaction confirmed\'">Check interaction</button></main></html>',
    );
  });
  ...
  try {
    ...
    await client.connect(
      new StdioClientTransport({ command, args, cwd: root, stderr: "inherit" }),
      { timeout: 30_000 },
    );
    ...
    console.log(
      "Playwright MCP passed: connection, navigation, accessibility snapshot, interaction, and PNG capture.",
    );
    console.log(
      "Evidence: artifacts/playwright/tooling-smoke.png. This does not validate the application or real media.",
    );
  } finally {
    await client.close();
