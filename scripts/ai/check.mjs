...
assert.deepEqual(
  ...
  pkg.devDependencies,
  "Lockfile dependencies differ from package.json",
);
assert.deepEqual(
  lock.packages[""].dependencies,
  pkg.dependencies,
  "Lockfile application dependencies differ from package.json",
);
for (const name of [
  "prettier",
  ...
  assert(pkg.devDependencies[name], `Missing formatting dependency: ${name}`);
}
for (const name of ["format", "format:check", "lint", "lint:fix", "prepare", "precommit"]) {
  assert(pkg.scripts[name], `Missing formatting or lint command: ${name}`);
}
for (const path of [
  ".prettierrc.json",
  ".prettierignore",
  ...
  assert(existsSync(resolve(root, path)), `Missing code quality configuration: ${path}`);
}
...
assert(
  existsSync(resolve(root, ".agents/skills/shadcn/SKILL.md")),
  "Missing official shadcn skill",
);
for (const [name, server] of Object.entries(config.mcp_servers)) {
  if (server.command === "node")
    assert(existsSync(resolve(root, server.args[0])), `Missing MCP launcher: ${name}`);
...
for (const filename of readdirSync(resolve(root, ".codex/agents"))) {
  ...
  for (const key of ["name", "description", "developer_instructions"])
    assert(agent[key], `${filename}: missing ${key}`);
...
for (const name of readdirSync(resolve(root, "scripts/ai")).filter((name) =>
  name.endsWith(".mjs"),
)) {
  const result = spawnSync(process.execPath, ["--check", resolve(root, "scripts/ai", name)], {
    encoding: "utf8",
  });
  assert.equal(result.status, 0, result.stderr);
}
const hook = spawnSync(process.execPath, [resolve(root, "scripts/ai/session-start.mjs")], {
  encoding: "utf8",
  cwd: root,
});
...
for (const path of [
  ...
  for (const match of read(path).matchAll(/\[[^\]]+\]\(([^)]+)\)/g)) {
    ...
    assert(
      existsSync(resolve(root, dirname(path), match[1].split("#")[0])),
      `${path}: broken link ${match[1]}`,
    );
  }
}
console.log(
  "Scaffolding checks passed: dependency lock, TOML, skill metadata, script syntax, hook context, planning references, and documentation links.",
);
