...
try {
  ...
  assert(tools.some((tool) => tool.name === 'search_items_in_registries'), 'Registry search tool is missing');
  ...
  assert.match(content, /^- button \(registry:ui\) \[@shadcn\]/m, 'The official registry did not return the button component');
  console.log(`shadcn MCP passed: connection, ${tools.length} tools discovered, and official button registry search.`);
  console.log('No components were installed. Application setup remains deferred to Phase 1.');
} finally {
  await client.close();
}
