...
try {
  ...
  if (values.help) {
    ...
    console.log('Outputs two private invitations. Give each test participant a different invitation. Reuse replaces that connection.');
  } else {
    ...
    const { invitations } = await createInvitationPair(process.env.INVITATION_SIGNING_SECRET ?? '', { room: values.room, validFor: Number(values.minutes) * 60 });
    const output = invitations.map((invitation, index) => {
