...
/** @param {string} secret */
function key(secret) {
  if (new TextEncoder().encode(secret).length < 32 || secret.startsWith("replace-"))
...
export async function createInvitation(
  ...
  if (
    ...
    (seat !== 1 && seat !== 2) ||
    !Number.isInteger(validFor) ||
    validFor < 60 ||
    validFor > 86400
  )
    throw new Error("Invalid invitation options");
  const invitation = await new SignJWT({ room, seat })
    ...
    .setJti(randomUUID())
    .setIssuedAt(now)
    .setExpirationTime(now + validFor)
    .sign(key(secret));
...
export async function createInvitationPair(secret, options = {}) {
  ...
  return {
    room: first.room,
    invitations: [first.invitation, second.invitation],
...
export function participantIdentity(room, seat) {
  if (!roomPattern.test(room) || (seat !== 1 && seat !== 2))
    throw new Error("Invalid participant place");
  return createHash("sha256")
    .update(`live-session-lab:participant:v1:${room}:${seat}`)
    .digest("hex");
}
...
export async function verifyInvitation(invitation, secret) {
  if (typeof invitation !== "string" || invitation.length > 2048)
  ...
  const { payload } = await jwtVerify(invitation, key(secret), {
    ...
    requiredClaims: ["exp", "iat", "jti", "room", "seat"],
    maxTokenAge: "24h",
  });
  if (
    typeof payload.room !== "string" ||
    !roomPattern.test(payload.room) ||
