...
import { verifyInviteCode } from "./invite-codes";
...
export async function admit(request: Request) {
  ...
  let input: ({ invitation: string } | { code: string }) & { displayName: string };
  try {
    ...
    const credential =
      keys === "displayName,invitation" && typeof fields.invitation === "string"
        ? { invitation: fields.invitation }
        : keys === "code,displayName" &&
            typeof fields.code === "string" &&
            fields.code.length <= 128
          ? { code: fields.code }
          : null;
    if (!credential || typeof fields.displayName !== "string") throw new Error();
    const displayName = fields.displayName.trim();
    if (!displayName || displayName.length > 40 || /[\p{Cc}\p{Cf}]/u.test(displayName))
      throw new Error();
    input = { ...credential, displayName };
  } catch {
    return reject(400, "invalid_request");
  }

  let validated: { room: string; seat: 1 | 2; expiresAt?: number };
  if ("code" in input) {
    let place;
    try {
      place = verifyInviteCode(input.code);
    } catch {
      return reject(503, "service_unavailable");
    }
    if (!place) return reject(401, "invalid_code");
    validated = place;
  } else {
    try {
      validated = await verifyInvitation(
        input.invitation,
        process.env.INVITATION_SIGNING_SECRET ?? "",
      );
    } catch {
      return reject(401, "invalid_invitation");
    }
  }
  ...
  try {
    ...
    const token = new AccessToken(apiKey, secret, {
      identity: participantIdentity(validated.room, validated.seat),
    ...
    if (validated.expiresAt !== undefined && validated.expiresAt <= Math.floor(Date.now() / 1000))
      return reject(401, "invalid_invitation");
