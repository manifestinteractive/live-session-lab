...
test("browser camera denial gives a usable recovery message", async ({ page, context }) => {
  ...
  await cdp.send("Browser.setPermission", { permission: { name: "camera" }, setting: "denied", origin: "http://localhost:3100", browserContextId: targetInfo.browserContextId });
  ...
  await expect(page.getByRole("alert", { name: "Action needed" })).toBeVisible();
  await expect(page.getByRole("alert", { name: "Action needed" })).toContainText("Camera access was blocked");
