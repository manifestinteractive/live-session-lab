...
for (const [errorName, message] of [
  ...
  test(`recovers the controls after ${errorName} without exposing browser error text`, async ({ page }) => {
    ...
    await page.evaluate(name => {
      ...
      Object.defineProperty(MediaDevices.prototype, "getUserMedia", { configurable: true, value: async () => { throw new DOMException("synthetic-private-device-detail", name); } });
      Object.defineProperty(navigator.mediaDevices, "getUserMedia", { configurable: true, value: async () => { throw new DOMException("synthetic-private-device-detail", name); } });
    }, errorName);
    await page.getByRole("button", { name: "Enable camera", exact: true }).click();
    const alert = page.getByRole("alert", { name: "Action needed" });
...
test("device refresh does not request capture and diagnostics expose no credentials", async ({ page }) => {
  ...
  const response = await page.goto("/call#invite=synthetic-invalid-invitation");
  ...
  expect(await page.evaluate(() => (window as unknown as { __captureCalls: number }).__captureCalls)).toBe(0);
  await expect(page.locator("body")).not.toContainText("synthetic-invalid-invitation");
});

test("a stalled admission request can be cancelled without losing the invitation", async ({ page }) => {
