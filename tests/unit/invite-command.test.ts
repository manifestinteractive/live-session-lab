...
it("creates two separate invitations for the same room without provider access", async () => {
  ...
  for (const url of urls) {
    expect(url.origin + url.pathname).toBe("https://example.test/");
    expect(url.search).toBe("");
    const validated = await verifyInvitation(
...
it("does not output credentials when operator configuration is invalid", () => {
  const result = spawnSync(process.execPath, ["scripts/operator/invite.mjs"], {
    env: {
      NODE_ENV: "test",
      PATH: process.env.PATH,
      APP_ORIGIN: "https://example.test",
      INVITATION_SIGNING_SECRET: "short",
    },
    encoding: "utf8",
  });
